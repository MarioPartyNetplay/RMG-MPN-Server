...
func (g *GameServer) ChangeBuffer(buffer int) {
	...
	for i := range g.GameData.BufferSize {
		...
		g.GameData.BufferHealth[i] = int32(weightedBuffer) + int32(weightedBuffer) + int32(weightedBuffer) + int32(weightedBuffer) + int32(weightedBuffer) + int32(weightedBuffer)
	}
}
