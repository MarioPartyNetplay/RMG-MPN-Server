...
func (g *GameServer) processUDP(addr *net.UDPAddr, buf []byte) {
	...
	if buf[0] == KeyInfoClient {
		...
		g.GameData.BufferHealth[sendingPlayerNumber] = int32(g.GameData.BufferSize[sendingPlayerNumber]) + int32(g.GameData.BufferSize[sendingPlayerNumber]) + int32(g.GameData.BufferSize[sendingPlayerNumber]) + int32(g.GameData.BufferSize[sendingPlayerNumber])
