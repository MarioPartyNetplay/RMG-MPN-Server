...
func (g *GameServer) sendUDPInput(count uint32, addr *net.UDPAddr, playerNumber byte, spectator bool, sendingPlayerNumber byte) uint32 {
	...
	// Introduce delay if countLag is greater than a threshold
	if countLag > 5 { // Adjust the threshold as needed
		time.Sleep(time.Millisecond * 50) // Adjust the delay duration as needed
	}
...
func (g *GameServer) processUDP(addr *net.UDPAddr, buf []byte) {
	...
	if buf[0] == KeyInfoClient {
		...
		g.GameData.BufferHealth[sendingPlayerNumber] = int32(g.GameData.BufferSize[sendingPlayerNumber]) + int32(g.GameData.BufferSize[sendingPlayerNumber]) + int32(g.GameData.BufferSize[sendingPlayerNumber]) + int32(g.GameData.BufferSize[sendingPlayerNumber]) + int32(g.GameData.BufferSize[sendingPlayerNumber])
