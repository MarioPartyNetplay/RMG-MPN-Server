...
func (g *GameServer) sendUDPInput(count uint32, addr *net.UDPAddr, playerNumber byte, spectator bool, sendingPlayerNumber byte) uint32 {
	...
	countLag := uint32(0)       // Force countLag to always be 0
	...
	buffer[3] = uint8(countLag) // This will always be 0
...
func (g *GameServer) createUDPServer() error {
	...
	g.GameData.BufferSize = []uint32{1, 1, 1, 1}
	...
	g.GameData.CountLag = make([]uint32, 0)  //nolint:gomnd
